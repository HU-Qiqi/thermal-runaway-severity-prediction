...
class FTRC_Data:
    ...
    def process_battery_failure_databank(self): 
        ...
        self.append_cell_energy()
        self.append_bottom_vent()
    ...
    def append_cell_energy(self):
        """Reads in cell energy from cell info sheet and adds cell energy column according to cell desc.  """
        cell_characteristics = pd.read_excel(self.file_path, sheet_name=self.sheet_name_cell_characteristics, header=2, usecols=range(1,17))
        is_nan_energy = np.isnan(cell_characteristics['Cell-Energy-Wh'])
        cell_characteristics = cell_characteristics.loc[~is_nan_energy]

        cd_list = cell_characteristics['Cell-Description'].to_list()
        cc_list = cell_characteristics['Cell-Energy-Wh'].to_list()
        ...
        self.df['Cell-Energy-Wh'] = final_col_list
    
    def append_bottom_vent(self):
        """Bottom-Vent-Yes-No:Reads in cell bottom vent from cell info sheet and adds cell bottom vent column according to cell desc. """
        cell_characteristics = pd.read_excel(self.file_path, sheet_name=self.sheet_name_cell_characteristics, header=2, usecols=range(1,17))
        # is_nan_vent = np.isnan(cell_characteristics['Bottom-Vent-Yes-No'])
        # cell_characteristics = cell_characteristics.loc[~is_nan_vent]

        cd_list = cell_characteristics['Cell-Description'].to_list()
        cc_list = cell_characteristics['Bottom-Vent-Yes-No'].to_list()
        ...
        self.df['Bottom-Vent-Yes-No'] = final_col_list
